...
def init():
    ...
    model = StableDiffusionPipeline.from_pretrained("prompthero/openjourney", use_auth_token=HF_AUTH_TOKEN).to("cuda")
...
def make_scheduler(name):
    ...
    if name == 'k_euler_ancestral':
        return EulerAncestralDiscreteScheduler.from_config("prompthero/openjourney", subfolder="scheduler", use_auth_token=HF_AUTH_TOKEN)

    if name == 'k_euler':
        return EulerDiscreteScheduler.from_config("prompthero/openjourney", subfolder="scheduler", use_auth_token=HF_AUTH_TOKEN)

    if name == 'ddim':
        return DDIMScheduler.from_config("prompthero/openjourney", subfolder="scheduler", use_auth_token=HF_AUTH_TOKEN)

    if name == 'pndm':
        return PNDMScheduler.from_config("prompthero/openjourney", subfolder="scheduler", use_auth_token=HF_AUTH_TOKEN)
        
    return LMSDiscreteScheduler.from_config("prompthero/openjourney", subfolder="scheduler", use_auth_token=HF_AUTH_TOKEN)
